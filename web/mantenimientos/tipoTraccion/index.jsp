    ...
    <form id="frm_princ" method="POST" action="<s:property value="baseURL" /><s:property value="urlPaginacion" />">
        ...
        <div class="d-grilla" style="overflow: hidden;">
            ...
            <div class="d-content-grilla-body">
                <table border="0" cellpadding="0" cellspacing="0" style="">
                    <s:iterator value="listTipoTraccion">
                        <tr>
                            <td style="width: 24px;">
                                <input type="radio" name="idTipTrac" id="rbt_idTipTrac" value="<s:property value="idTipTrac" />" class="select_rec" />
                            </td>
                            <td style="width: 50px; text-align: center;">
                                <s:property value="idTipTrac" />
                            </td>
                            <td style="">
                                <s:property value="desTipTrac" />
                            </td>
                        </tr>
                    </s:iterator>
                </table>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#btn-add').click(function() {
            ...
            $('#frm_princ').attr('action','<s:property value="baseURL" /><s:url namespace="tipoTraccion" includeContext="false" action="adicionarTipoTraccion" />');
            $('#frm_princ').submit();
        });
        
        $('#btn-edit').click(function() {   
            post(
                '<s:property value="baseURL" /><s:url namespace="tipoTraccion" includeContext="false" action="vrfSeleccionTipoTraccion" />',
                $('#frm_princ').serialize(),
                function(resultado){
                    ...
                    else
                    {
                        $('#opcion_h1').val('M');
                        var href = $(location).attr('href');                        
                        var _varret = $('#nivBandeja_f').val()+'%'+href+'%'+$('#mtu_h1').val()+'%'+$('#mmo_h1').val()+'%'+$('#mop_h1').val()+'%'+$('#mni_h1').val()+'%'+$('#mod_h1').val()+'%'+$('#curPag_f').val()+'%'+$('#idTipTrac').val()+'|';
                        $('#varReturn_f').val($('#varReturn_f').val()+_varret);
                        
                        $('#frm_princ').attr('action','<s:property value="baseURL" /><s:url namespace="tipoTraccion" includeContext="false" action="modificarTipoTraccion" />');
                        $('#frm_princ').submit();
                    }
                },
                1
            ...
            post(
                ...
                function(resultado){
                    ...
                    else
                    {
                        $('#opcion_h1').val('C');
                        post(
                            '<s:property value="baseURL" /><s:url namespace="tipoTraccion" includeContext="false" action="eliminarTipoTraccion" />',
                            $('#frm_princ').serialize(),
                            function(resultado1)
                            {
                                            ...
                                            "Confirmar":function(){
                                                ...
                                                post(
                                                    ...
                                                    function(resultado2)
                                                    {
                                                        ...
                                                        {
                                                            $('#DIVeliminar').dialog('close');
                                                            $('#frm_princ').attr('action','<s:property value="baseURL" /><s:url namespace="tipoTraccion" includeContext="false" action="TipoTraccion" />');
                                                            $('#frm_princ').submit();
                                                        }
                                                    },
                                                    4
