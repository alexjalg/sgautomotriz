    ...
    <form id="frm_princ" method="POST" action="<s:property value="baseURL" /><s:property value="urlPaginacion" />">
        ...
        <div class="d-grilla" style="overflow: hidden;">
            ...
            <div class="d-content-grilla-body">
                <table border="0" cellpadding="0" cellspacing="0" style="">
                    <s:iterator value="listAmbUbi">
                        <tr>
                            <td style="width: 24px;">
                                <input type="radio" name="idAmbUbi" id="rbt_idAmbUbi" value="<s:property value="idAmbUbi" />" class="select_rec" />
                            </td>
                            <td style="width:60px; text-align: center;">
                                <s:property value="idAmbUbi" />
                            </td>
                            <td style="">
                                <s:property value="desAmbUbi" />
                            </td>
                            <td style="width:150px; text-align: center;">
                                <s:hidden name="codTipAmbUbi" />
                                <s:if test='%{codTipAmbUbi=="R"}'>
                                    Almacen Repuestos
                                </s:if>
                                <s:if test='%{codTipAmbUbi=="T"}'>
                                    Taller Servicios
                                </s:if>
                                <s:if test='%{codTipAmbUbi=="V"}'>
                                    Almacen Vehiculos
                                </s:if>
                                <s:if test='%{codTipAmbUbi=="I"}'>
                                    Almacen Virtual
                                </s:if>
                            </td>
                            <td style="width:120px; text-align: center;">
                                <s:property value="desCorAmbUbi" />
                            </td>
                        </tr>
                    </s:iterator>
                </table>
...
<script type="text/javascript">
    ...
    <s:property value="jsPaginacion" escape="false" />
        ...
        $('#btn-add').click(function() {
            ...
            $('#frm_princ').attr('action', '<s:property value="baseURL" /><s:url namespace="ambienteUbicacion" includeContext="false" action="adicionarAmbienteUbicacion" />');
            $('#frm_princ').submit();
        });
        $('#btn-edit').click(function() {
            post(
                    '<s:property value="baseURL" /><s:url namespace="ambienteUbicacion" includeContext="false" action="vrfSeleccionAmbienteUbicacion" />',
                    $('#frm_princ').serialize(),
                    function(resultado) {
                        ...
                        else
                        {
                            ...
                            var _varret = $('#nivBandeja_f').val() + '%' + href + '%' + $('#mtu_h1').val() + '%' + $('#mmo_h1').val() + '%' + $('#mop_h1').val() + '%' + $('#mni_h1').val() + '%' + $('#mod_h1').val() + '%' + $('#curPag_f').val() + '|';
                            $('#varReturn_f').val($('#varReturn_f').val() + _varret);
                            $('#frm_princ').attr('action', '<s:property value="baseURL" /><s:url namespace="ambienteUbicacion" includeContext="false" action="modificarAmbienteUbicacion" />');
                            $('#frm_princ').submit();
                        }
                    },
                    1
            ...
            post(
                    ...
                    function(resultado) {
                        ...
                        else
                        {
                            $('#opcion_h1').val('C');
                            post(
                                    '<s:property value="baseURL" /><s:url namespace="ambienteUbicacion" includeContext="false" action="eliminarAmbienteUbicacion" />',
                                    $('#frm_princ').serialize(),
                                    function(resultado1)
                                    {
                                                    ...
                                                    "Confirmar": function() {
                                                        ...
                                                        post(
                                                                ...
                                                                function(resultado2)
                                                                {
                                                                    ...
                                                                    {
                                                                        $('#DIVeliminar').dialog('close');
                                                                        $('#frm_princ').attr('action', '<s:property value="baseURL" /><s:url namespace="ambienteUbicacion" includeContext="false" action="AmbienteUbicacion" />');
                                                                        $('#frm_princ').submit();
                                                                    }
                                                                },
                                                                4
    ...
    $('#btn-ubicaciones').click(function() {
        post(
                '<s:property value="baseURL" /><s:url namespace="ambienteUbicacion" includeContext="false" action="vrfSeleccionAmbienteUbicacion" />',
                $('#frm_princ').serialize(),
                function(resultado) {
                    ...
                    else
                    {
                        ...
                        $('#varReturn_f').val($('#varReturn_f').val() + _varret);
                        $('#curPag_f').val(1);
                        $('#frm_princ').attr('action', '<s:property value="baseURL" /><s:url namespace="ubicacionAmbiente" includeContext="false" action="UbicacionAmbiente" />');
                        $('#frm_princ').submit();
                    }
                },
                1
